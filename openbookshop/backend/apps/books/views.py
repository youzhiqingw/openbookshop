...
class BookListView(ListAPIView):
    ...
    def get_queryset(self):
        queryset = Book.objects.filter(is_on_sale=True).select_related(
            "merchant", "category"
        )
...
class BookDetailView(RetrieveAPIView):
    ...
    def get_queryset(self):
        return Book.objects.filter(is_on_sale=True).select_related(
            "merchant", "category"
        )
...
class MerchantBookListView(ListAPIView):
    ...
    def get_queryset(self):
        return Book.objects.filter(merchant=self.request.user.merchant).select_related(
            "category"
        )
...
class AdminBookListView(ListAPIView):
    ...
    def get_queryset(self):
        queryset = Book.objects.select_related("merchant", "category").order_by(
            "-created_at"
        )
...
class AdminLowStockView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Book.objects.filter(stock__lte=F("warning_stock"))
...
class BookReviewListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Review.objects.filter(book_id=book_id, is_approved=True)
            .select_related("user")
            .order_by("-created_at")
        )
    ...
    def get_queryset(self):
        ...
        if is_sensitive is not None:
            queryset = queryset.filter(
                is_sensitive=is_sensitive in ("true", "1", "True")
            )
        return queryset
...
class MerchantReviewListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Review.objects.filter(book__merchant=self.request.user.merchant)
