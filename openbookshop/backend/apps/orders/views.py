...
class CartUpdateView(UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        return success_response(
            data=CartSerializer(instance).data, message="购物车已更新"
        )
...
class OrderCreateView(APIView):
    ...
    def post(self, request):
        ...
        cart_qs = Cart.objects.filter(user=request.user).select_related(
            "book", "book__merchant"
        )
        if cart_item_ids:
        ...
        # Lock books with select_for_update to prevent overselling
        ...
        for item in cart_items:
            ...
            display_title = (
                book.title if book else (item.book.title if item.book else "该图书")
            )
        ...
        OperationLog.objects.create(
            ...
            module="orders",
            detail=(
                f"用户 {request.user.username} 创建订单"
                f" {order.order_no}，金额 ¥{total_amount}"
            ),
...
class OrderCancelView(APIView):
    ...
    def post(self, request, pk):
        ...
        items_with_books = list(
            order.items.filter(book__isnull=False).select_related("book")
        )
        if items_with_books:
...
class OrderPayView(APIView):
    ...
    def post(self, request, pk):
        ...
        order.save(
            update_fields=["payment_method", "mock_payment_id", "pay_url", "updated_at"]
        )
...
class OrderPayCallbackView(APIView):
    ...
    def post(self, request, pk):
        ...
        if payment_status == "success":
            ...
            return success_response(
                data={"status": "pending"}, message="支付处理中，请稍候"
            )
        else:
...
class MerchantOrderShipView(APIView):
    ...
    def post(self, request, pk):
        ...
        order_ids = OrderItem.objects.filter(merchant=merchant).values_list(
            "order_id", flat=True
        )
        try:
        ...
        OperationLog.objects.create(
            ...
            module="orders",
            detail=(
                f"商家 {merchant.store_name} 发货订单"
                f" {order.order_no}，快递单号 {order.tracking_number}"
            ),
            ip_address=request.META.get("REMOTE_ADDR"),
            user_agent=request.META.get("HTTP_USER_AGENT", ""),
        )
...
class MerchantFinanceListView(ListAPIView):
    ...
    def get_queryset(self):
        queryset = (
            FinanceRecord.objects.filter(merchant=self.request.user.merchant)
...
class AdminStatisticsView(APIView):
    ...
    def get(self, request):
        ...
        total_revenue = (
            FinanceRecord.objects.filter(type="income").aggregate(total=Sum("amount"))[
                "total"
            ]
            or 0
        )
        ...
        revenue_map = {
            str(item["date"]): float(item["total"]) for item in daily_revenue
        }
        daily_data = [
            {
                "date": d,
...
class MerchantAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        # 收入统计
        total_revenue = (
            FinanceRecord.objects.filter(merchant=merchant, type="income").aggregate(
                total=Sum("amount")
            )["total"]
            or 0
        )
        ...
        daily_data = [
            {"date": d, "revenue": revenue_map.get(d, 0.0)} for d in date_range
        ]
