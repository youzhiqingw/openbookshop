...
class CartAPITestCase(APITestCase):
    ...
    def test_add_to_cart(self):
        ...
        self.assertTrue(
            Cart.objects.filter(user=self.customer, book=self.book).exists()
        )
...
class OrderAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.cart_item = Cart.objects.create(
            user=self.customer, book=self.book, quantity=2
        )
    ...
    def test_pay_order(self):
        ...
        resp = self.client.post(
            f"/api/v1/orders/{order.id}/pay/", {"payment_method": "mock"}, format="json"
        )
        self.assertEqual(resp.status_code, 200)
...
class FinanceStatisticsAPITestCase(APITestCase):
    ...
    def test_merchant_finance_excludes_other_merchants(self):
        other_user = User.objects.create_user(
            "fin_other_m", password="pass", role="merchant"
        )
