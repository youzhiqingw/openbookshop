...
test.describe('Menu Management i18n', () => {
  test('menu page has zero [intlify] warnings in zh-CN and en', async ({ authenticatedPage }) => {
    ...
    expect(
      zhWarnings,
      `zh-CN [intlify] warnings on /menu:\n${zhWarnings.map((w) => w.text).join('\n')}`
    ).toHaveLength(0);
    expect(
      enWarnings,
      `en [intlify] warnings on /menu:\n${enWarnings.map((w) => w.text).join('\n')}`
    ).toHaveLength(0);
  });
});
